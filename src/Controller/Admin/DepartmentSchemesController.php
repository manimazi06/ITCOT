...
class DepartmentSchemesController extends AppController
{
	...
	public function add()
	{
		...
		if ($this->request->is('post')) { //echo "<pre>"; print_r($this->request->getData());  exit();
			...
			$deptscheme_check  = $this->DepartmentSchemes->find('all')->where(['DepartmentSchemes.name'=>$departscheme->name,'DepartmentSchemes.department_id'=>$departscheme->department_id])->count();


			if($deptscheme_check>0){
				$this->Flash->error(__('The Department and name already present. Please, try again.'));
				return $this->redirect(['action' => 'index']);

		}else{
			$this->DepartmentSchemes->save($departscheme);
	...
	public function edit($id = null)
	{
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			$deptscheme_check  = $this->DepartmentSchemes->find('all')->where(['DepartmentSchemes.name'=>$departscheme->name,'DepartmentSchemes.department_id'=>$departscheme->department_id,'DepartmentSchemes.id !='=>$id])->count();
