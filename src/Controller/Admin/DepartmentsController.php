...
class DepartmentsController extends AppController
{
	...
	public function add()
	{
		...
		if ($this->request->is('post')) { 
			...
			$dept_check  = $this->Departments->find('all')->where(['Departments.name'=>$department->name,'Departments.scheme_type_id'=>$department->scheme_type_id])->count();

			if($dept_check>0){
				$this->Flash->error(__('The Scheme type and department name already present. Please, try again.'));
				return $this->redirect(['action' => 'index']);

		}else{
			$this->Departments->save($department);
			$this->Flash->success(__('The Department details has been saved.'));
			return $this->redirect(['action' => 'index']);
		}
		...
		if ($this->request->is(['patch', 'post', 'put'])) { // echo "<pre>";  print_r($this->request->getData());  exit();
			...
			if($department['scheme_type_id']==2){
			 ...
			 if ($departmentpdf->getClientFilename() != '' && $departmentpdf->getError() == 0) {
               // $auth_id                                      = $this->Auth->user('id');
                ...
                $department->file_upload             = $this->request->getData('file_upload1');;
            } 	
		   ...
		   if ($image_upload->getClientFilename() != '') {
			   ...
			   if ($name != '' && $error == 0) {
				   ...
				   $fileExt                                   = $array[count($array) - 1];
				   $current_time                              = date('Y_m_d');
				   $newfile                                   = "Union_".$department['id'].'.'.$fileExt;
				   $tempFile                                  = $tmpName;
				   $targetPath                                = 'uploads/Govt_schemes/Union/';
				   $targetFile                                = $targetPath . $newfile;
			...
			$department->name                = $this->request->getData('name');
			$department->description         = $this->request->getData('description');
			$dept_check  = $this->Departments->find('all')->where(['Departments.name'=>$department->name,'Departments.scheme_type_id'=>$department->scheme_type_id,'Departments.id !='=>$id])->count();
