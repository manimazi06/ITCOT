...
// exit();
	...
	public function add()
	{
		...
		if ($this->request->is('post')) { //echo "<pre>"; print_r($this->request->getData());  exit();
			...
			$Msmedivisions_check  = $this->MsmeDivisions->find('all')->where(['MsmeDivisions.name'=>$Msmedivisions->name])->count();

			//echo "<pre>"; print_r($Msmedivisions_check);exit();

		if($Msmedivisions_check>0){
				$this->Flash->error(__('The MSME Name already present. Please, try again.'));
				//return $this->redirect(['action' => 'index']);

		}else{
			$this->MsmeDivisions->save($Msmedivisions);
	...
	public function edit($id = null)
	{
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			$Msmedivisions_check  = $this->MsmeDivisions->find('all')->where(['MsmeDivisions.name'=>$Msmedivisions->name,'MsmeDivisions.id!='=>$id])->count();
