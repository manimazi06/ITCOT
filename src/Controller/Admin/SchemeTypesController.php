...
//echo "<pre>"; print_r($this->request->getData());  exit();
			...
			$scheme->description         = $this->request->getData('description');
				...
				$scheme_check  = $this->SchemeTypes->find('all')->where(['SchemeTypes.name'=>$scheme->name,'SchemeTypes.scheme_id'=>$scheme->scheme_id])->count();

				if($scheme_check>0){
					$this->Flash->error(__('The Scheme type and name already present. Please, try again.'));
					//return $this->redirect(['action' => 'index']);
	
			}else{
				$this->SchemeTypes->save($scheme);
	...
	public function edit($id = null)
	{
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			$scheme_check  = $this->SchemeTypes->find('all')->where(['SchemeTypes.name'=>$scheme->name,'SchemeTypes.scheme_id'=>$scheme->scheme_id,'SchemeTypes.id !='=>$id])->count();
