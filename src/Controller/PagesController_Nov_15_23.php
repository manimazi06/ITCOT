...
$pressReleases              = $connection->execute($query)->fetchAll('assoc');
    ...
    public function projectreport(){
	 ...
	 public function digitallibrary(){
		...
		$schemeType         = $this->SchemeTypes->find('all')->toArray();
		// $schemeType_2         = $this->SchemeTypes->find('all')->where(['SchemeTypes.id'=>2])->toArray();

		// echo '<pre>';
		// print_r($schemeType);
		// exit();
		
      $this->set(compact('user','digiregistration','schemeType'));    
	}
	
	public function ajaxdigitallogs($loc=null,$scheme_type=null,$dept=null,$dept_scheme=null)
	{

		
				$decode=base64_decode($loc);
		
//print_r($scheme_type); exit();

		$this->loadModel('DigitalLibraryLogs');

			$digitalLibraryLogs = $this->DigitalLibraryLogs->newEmptyEntity();

			$digitalLibraryLogs->scheme_type_id         =$scheme_type;
...
exit();
	...
	public function unionschemes($schemeType=null){
	    ...
	    $departments_dropdown         = $this->Departments->find('list',['keyField' => 'id', 'valueField' => 'name'])->where(['Departments.scheme_type_id'=>1])->toArray();	

		// echo '<pre>';
		// print_r($departments_dropdown);
		// exit();
		...
		//echo "<pre>"; print_r($union_schemes_2); exit();
		  $this->set(compact('union_schemes_1','union_schemes_2','departments_dropdown','schemeType'));
	}
	
	public function schemeview($schemeType = null,$id=null){
	...
	public function msmedivisions($id = null){
		...
		$msmedivisions  = $this->MsmeDivisions->find('all')->where(['MsmeDivisions.is_active'=>1,'MsmeDivisions.department_id'=>$id])->toArray();  	
	...
	public function schemedetailview($id = null,$scheme_id = null){		
		...
		//echo "<pre>";  print_r($schemes); exit();		
		$department = $this->Departments->find('list',['keyField' => 'id',  'valueField' => 'file_upload'])->toArray();	   
		  $this->set(compact('schemes','id','department','scheme_id'));		
	}
	
	public function stateschemes($schemeType=null){
		$this->viewBuilder()->setLayout('layout');
		
		$this->loadModel('Departments');
		$state_schemes_1   = $this->Departments->find('all')->where(['Departments.scheme_type_id'=>2])->limit(9)->toArray();
	    $first_id          = $this->Departments->find('list')->where(['Departments.scheme_type_id'=>2])->limit(9)->toArray();	
	    $second_id         = $this->Departments->find('list')->where(['Departments.scheme_type_id'=>2,'Departments.id NOT IN'=>$first_id])->limit(9)->toArray();	
		$departments_dropdown         = $this->Departments->find('list',['keyField' => 'id', 'valueField' => 'name'])->where(['Departments.scheme_type_id'=>2])->toArray();	
		...
		if($first_id != ''){
		   $state_schemes_2  = $this->Departments->find('all')->where(['Departments.scheme_type_id'=>2,'Departments.id NOT IN'=>$first_id ])->limit(9)->toArray();	
		}
		
		if($combined != ''){
		   $state_schemes_3  = $this->Departments->find('all')->where(['Departments.scheme_type_id'=>2,'Departments.id NOT IN'=>$combined ])->limit(10)->toArray();	
		}
		
		 $this->set(compact('state_schemes_1','state_schemes_2','state_schemes_3','departments_dropdown','schemeType'));		
	}
...
// exit();
	...
	public function ajaxstatuscheck($val=null,$appln_no=null){
	...
	//	$connection      = ConnectionManager::get('default'); 
 		//  if ($this->request->is(['patch', 'post', 'put'])){ //echo "<pre>"; print_r($this->request->getData());  exit(); 
				...
				//print_r($pro_no);  exit(); 
				if($val==1){		  
				$projectcount  = $this->Projects->find('all')->count();
				$project       = $this->Projects->find('all')->toArray();
				}
				// 	echo '<pre>';
				// print_r($project); exit();
		//  }

		  $status=array(1=>'Project Report',2=>'Digilibrary',3=>'VirtualcfoRegistrations',4=>'Eprcertification',5=>'ComplianceServices');
		$this->set(compact('project','projectcount','pro_no','status','val'));		
	}
	...
	public function digitalpayment(){
	  ...
	  if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>"; print_r($this->request->getData());  exit();  		
		...
		//    if($digiregistration['payment_status']==1){
		//    $digitalLibraryLogs = $this->DigitalLibraryLogs->newEmptyEntity();
		...
		//    $this->DigitalLibraryLogs->save($digitalLibraryLogs);
