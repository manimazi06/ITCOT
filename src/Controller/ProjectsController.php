...
class ProjectsController extends AppController
{
	...
	// private $prefix = "ENC:";
	 ...
	 function encryption($data){
        $key='fWe6qWEJCWb2gcH4+APJL1YmtZmn0NVS';
        $iv ='c1aeB65F17A1c7f3';    
        $cipher = "AES-256-CBC";
        $plaintext =$data;
	 ...
	 function decryption($enc_data){
        $key='fWe6qWEJCWb2gcH4+APJL1YmtZmn0NVS';
        $iv ='c1aeB65F17A1c7f3';
        $ciphertext=$enc_data;
        $cipher = "AES-256-CBC";
        $ivlen = openssl_cipher_iv_length($cipher);
        $iv = 'c1aeB65F17A1c7f3';//openssl_random_pseudo_bytes($ivlen);
        $tag="";
	...
	public function projectreport(){
		...
		// $projects_first = $this->Projects->find('all')->where(['Projects.is_active'=>1,'Projects.created_by'=>$this->Auth->User('id')])->first();
		//$str=strval($projects[0]['id']);
			//$encode=base64_encode($str);
			//echo "<pre>"; print_r($str);  exit();
		$this->set(compact('projects','encode'));   
	}
	...
	{  
		...
		//echo "<pre>"; print_r(gettype($id));  exit();
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>"; print_r($this->request->getData());  exit();
			...
			if ($this->Projects->save($project)) {
				...
				//$insert_encode=strval($insert_id);
				// echo '<pre>';
				// print_r(gettype($insert_id));
				// exit();

				$insert_type=strval($insert_id);
				// echo '<pre>';
				// print_r(gettype($insert_type));
				// exit();
				$insert= base64_encode($insert_type);
				$id= $insert;
				// echo '<pre>';
			   ...
			   $project1->reg_no      = 'R'.date('Ym').sprintf('%03d',$id);
			   $ProjectTable->save($project1);
				...
				return $this->redirect(['action' => 'entitydetails',$id]);
		...
		$categories  = $this->Categories->find('list')->where(['Categories.is_active'=>1])->toArray();	
		$educations   = $this->Educations->find('list')->where(['Educations.is_active'=>1])->toArray();	
		
	  $this->set(compact('project', 'prefix','categories','educations','id','insert'));
	}
		...
		if ($this->request->is(['patch', 'post', 'put'])) { 
		   ...
		   // $timedate = TimeConversion::ConvertServerTimezoneToLocalTimezone(date("Y-m-d H:i:s"),'Asia/Calcutta');
			...
			if ($this->Projects->save($project)) {
				...
				$insert_id = $id;
				$insert_type=strval($insert_id);
				$insert_encode= base64_encode($insert_type);	
				//echo "<pre>"; print_r($insert_id);  exit();		
				return $this->redirect(['action' => 'educationdetails',$insert_encode]);
			}
			$this->Flash->error(__('The Projects could not be saved. Please, try again.'));
		}
	...
	public function educationdetails($pid = null)
	{	
		// echo '<pre>';
		// print_r($pid);
		// exit();

		$decode=(base64_decode($pid));
		// echo '<pre>';
		// print_r(gettype($decode));
		// exit();
		//$id=intval($decode);
		$id=$decode;
		// echo '<pre>';
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>"; print_r($this->request->getData());  exit();
		   ...
		   foreach($this->request->getData('educationqualification') as $key => $value){
			   if($value['edu_qualification_id'] != ''){
				...
				//  echo "<pre>";
				//  print_r(($educationqualification));  exit();
          ...
          // exit();		   
		   ...
		   $insert_encode= base64_encode($insert_type);
			...
			return $this->redirect(['action' => 'manufacturingdetails',$insert_encode]);	
	  ...
	  $this->set(compact('project', 'prefix','categories','educations','insert_encode','education_count','training_count','work_count','education_details','training_details','work_details','id'));
	}
	...
	{	
		...
		//    exit();
		   ...
		   $insert_encode= base64_encode($insert_type);	
			...
			return $this->redirect(['action' => 'manpowerdetails',$insert_encode]);  	 	
		} 

     		$units  = $this->Units->find('list',['keyField' => 'id',  'valueField' => 'description'])->toArray();
		
		
	  $this->set(compact('units','project', 'prefix','categories','production','id','production_count','machinery_count','raw_count','production_details','machinery_details','raw_details','id'));
	}
	...
	{	
		...
		foreach($this->request->getData('manpower') as $key => $value1){
          ...
          if($project['step_count'] <= 4){
			...
			$insert_type=strval($insert_id);		
			$insert_encode= base64_encode($insert_type);
			return $this->redirect(['action' => 'projectcostdetails',$insert_encode]);		
		} 
	  ...
	  $this->set(compact('project', 'prefix','categories','production','id','utilities_count','manpower_count','utilities','manpower','units','utilities_particulars','utilities','manpower_particulars','id'));
	}
	...
	{	
		...
		if ($this->request->is(['patch', 'post', 'put'])) {  //echo "<pre>"; print_r($this->request->getData());  exit(); 
           ...
           if($project['step_count'] <= 5){
			...
			return $this->redirect(['action' => 'profitabilitydetails',$insert_encode]);		
		}
	  ...
	  $this->set(compact('units','project', 'prefix','categories','production','id','production_count','training_count','fixed_count','working_count','fixed_capital','working_capital','means_Finance','fixed','capital_drop','fixedCapitals','id'));
	}
	...
	//   exit();
	    ...
	    if ($this->request->is(['patch', 'post', 'put'])) { // echo "<pre>"; print_r($this->request->getData());  exit();  
		   ...
		   $insert_id = $id;
		   $insert_encode= base64_encode($insert_id);			
		   return $this->redirect(['action' => 'suppliementarydetails',$insert_encode]);	
	   ...
	   $this->set(compact('project','id','manufacturing_expenses','means_Finance','finance_dropdown','financemeans_count','means_Finance','id'));	
	}
	  ...
	  $this->viewBuilder()->setLayout('layout');
	   $project  = $this->Projects->find('all')->where(['Projects.id'=>$id])->first();		   
	    ...
	    if ($this->request->is(['patch', 'post', 'put'])) { // echo "<pre>"; print_r($this->request->getData());  exit();  
		   ...
		   $insert_id = $id;	
		   $insert_encode= base64_encode($insert_id);		
		   return $this->redirect(['action' => 'referencedetails',$insert_encode]);	
		}		
	  ...
	  $this->loadModel('UserReferences');
       ...
       $reference_count       = $this->UserReferences->find('all')->where(['UserReferences.project_id'=>$id])->count();
	    ...
	    if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>"; print_r($this->request->getData());  exit();  
           ...
           $insert_id = $id;	
		   $insert_encode= base64_encode($insert_id);		
	       return $this->redirect(['action' => 'paymentdetails',$insert_encode]);			   
		}		
	  ...
	  $this->loadModel('Razorpaytransactions');
	   ...
	   $payment_Details  = $this->PaymentDetails->find('all')->toArray();  			 	   
	    ...
	    if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>"; print_r($this->request->getData());  exit(); 
		   ...
		   $insert_encode= base64_encode($insert_id);
		//    $id=$insert_encode;
		   	return $this->redirect(['action' => 'createorderapi',$insert_encode]);			   
          ...
          "url"        => $decoded->links[1]->href
			  ...
			  // echo "<pre>"; print_r($paymentorder);  exit();		
		       if($this->Billdesktransactions->save($transaction)){
					...
					// echo "<pre>"; print_r(gettype($insert_id));  exit();	
					 $str=strval($insert_id);
					 $insert_encode= base64_encode($str);
               	    return $this->redirect(['action' => 'bdpaymentresponse',$insert_encode]);			   
   ...
   public function bdresponse(){	
	  ...
	  // $user_id = $this->request->getSession()->read('User.id');
	   ...
	   //echo "<pre>"; print_r($user_id); exit();
		...
		// echo "<pre>"; print_r($data); exit(); 
			   ...
			   //echo "<pre>"; print_r($billdeskpaymentlog);  exit();		
			...
			//    exit();
			 ...
			 echo '<script>window.location.href = "https://itcot.demodev.in/projects/paymentresponseview/'.$encode.'";</script>'; 
		}	  
      exit();		
   }
	...
	public function ajaxutilities($j = null){	
