...
class ProjectsController extends AppController
{
	...
	{	
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>"; print_r($this->request->getData());  exit();
		   ...
		   foreach($this->request->getData('special') as $key1 => $value1){
			   ...
			   $training->from_date                = date('Y-m-d',strtotime($value1['from_date'])) ? date('Y-m-d',strtotime($value1['from_date'])) :'';
			   $training->to_date                  = date('Y-m-d',strtotime($value1['to_date'])) ? date('Y-m-d',strtotime($value1['to_date'])): '';
          ...
          // exit();		   
		   ...
		   foreach($this->request->getData('work') as $key2 => $value2){			  
			   ...
			   $work->from_date                = date('Y-m-d',strtotime($value2['from_date'])) ? date('Y-m-d',strtotime($value2['from_date'])):'';
			   $work->to_date                  = date('Y-m-d',strtotime($value2['to_date']))?  date('Y-m-d',strtotime($value2['to_date'])):'';
	...
	{	
		...
		if ($this->request->is(['patch', 'post', 'put'])) { //echo "<pre>"; print_r($this->request->getData());  exit();
		   ...
		   if($this->request->getData('electricity_requirement') != ''){
			   if($this->request->getData('utility_id') != ''){
			     ...
			     $utilities->updated_on               = date('Y-m-d H:i:s');
			   }else{
		   ...
		   if($this->request->getData('skilled_no') != ''){
			   if($this->request->getData('manpower_id') != ''){
			     ...
			     $manpower->updated_on               = date('Y-m-d H:i:s');
			   }else{
	  ...
	  $this->set(compact('project', 'prefix','categories','production','id','utilities_count','manpower_count','utilities','manpower','units','utilities_particulars'));
	}
		...
		if ($this->request->is(['patch', 'post', 'put'])) {  //echo "<pre>"; print_r($this->request->getData());  exit(); 
            ...
            if($this->request->getData('raw_duration') != ''){
			   if($this->request->getData('working_capital_id') != ''){
			     ...
			     $working_capital->updated_on               = date('Y-m-d H:i:s');
			   }else{
	    ...
	    if ($this->request->is(['patch', 'post', 'put'])) { // echo "<pre>"; print_r($this->request->getData());  exit();  
		   ...
		   if($this->request->getData('capital_finance_value') != ''){
			   if($this->request->getData('mean_finance_id') != ''){
			     ...
			     $means_Finance->updated_on               = date('Y-m-d H:i:s');
			   }else{
