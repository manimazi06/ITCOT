	...
	{	
	    ...
	    $this->loadModel('WorkExperiance');
		 ...
		 if($education_count != 0){
		  $education_details  = $this->EducationQualifications->find('all')->where(['EducationQualifications.project_id'=>$id])->toArray();	
		 }
		...
		if ($this->request->is(['patch', 'post', 'put'])) {  //echo "<pre>"; print_r($this->request->getData());  exit();
		   ...
		   foreach($this->request->getData('education') as $key => $value){
			   if($value['edu_qualification_id'] != ''){
			     ...
			     $education  = $this->EducationQualifications->patchEntity($education1, $this->request->getData());
				 $education->updated_by               = $this->Auth->user('id');
			     $education->updated_on               = $timedate;
			   }else{
			     $education = $this->EducationQualifications->newEmptyEntity();
                 $education->created_by               = $this->Auth->user('id');
			     $education->created_on               = $timedate;				
			   }
			   $education->project_id               = $id;
			   $education->education_id             = $value['education_id'];
			   $education->institute                = $value['institute'];
			   $education->major_subject            = $value['major_subject'];
			   $education->year_passing             = $value['year_passing'];			  
			   $this->EducationQualifications->save($education);
		   }		
