            ...
            <div class="table-responsive">
		     <table class="table table-hover table-bordered table-advanced display" style="width: 100%" id="example4">
				...
				<tbody>
					<?php
					$sno = 1;
					foreach ($complianceServices as $complianceService) : 
					?>
						<tr>
							<td align="center"><?= $sno++; ?></td>
							<td><?= $complianceService->name ?></td>
							<td><?= $complianceService->mobile_no ?></td>
							<td><?= $complianceService->email ?></td>
							<td><?= $complianceService->application_no ?></td>
							<td><?= $complianceService->project_name ?></td>
                            <td>
                                    <?php  if($complianceService['appln_status']==1){
                                        echo '<span style="color:green;">Approved</span>';
                                    }elseif($complianceService['appln_status']==2){
							...
							<td align="center">
								<?php echo $this->Html->link(('<i class="fas fa-eye"></i>'), ['action' => 'view', $complianceService->id], ['escape' => false, 'class' => 'btn waves-effect waves-light btn-light-primary text-primary btn-sm']); ?>
								<?php echo $this->Html->link(('<i class="fas fa-view">Status</i>'), ['action' => 'applnstatus', $complianceService->id], ['escape' => false, 'class' => 'btn waves-effect waves-light btn-light-primary text-success btn-sm']); ?>
								<?php echo $this->Html->link(('<i class="fas fa-view">Preview</i>'), ['action' => 'preview', $complianceService->id], ['escape' => false, 'class' => 'btn waves-effect waves-light btn-light-primary text-success btn-sm']); ?>
							</td>
						</tr>
					<?php endforeach; ?>
					</tbody>
